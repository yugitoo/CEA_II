...
plota_boxplot = function(col_box){
  ...
  for (i in col_box) {
    ...
    # adjbox(split(j,bd$grupo), col=cores, main = paste0(i," por grupo"))
        adjbox(j ~ bd$grupo,col=cores, main = paste0(i," por grupo"))
...
plota_disp = function(col_disp, col_resp){
  ...
  for (i in col_disp) {
    ...
    for (k in col_resp) {
      ...
      plot(j,l,pch = 16,  main = paste0(i," por ", k), col=cores, xlab=i,ylab=k) 
      legend("topright", inset=.05,
   c("dirige","n dirige","controle"), fill=cores, horiz=FALSE)
  }
}
}
```
...
#### q4,q5,q7,q8
```{r}
# tabelas com proporcoes por grupo
round(prop.table(table(bd$q4,bd$grupo),2),2)*100
round(prop.table(table(bd$q5,bd$grupo),2),2)*100
round(prop.table(table(bd$q7,bd$grupo),2),2)*100
round(prop.table(table(bd$q8,bd$grupo),2),2)*100
round(prop.table(table(bd$q9,bd$grupo),2),2)*100
```
...
via_sem3<- table(bd$q7,bd$deixou_a_via_sem_distrator)
via_sem3
...
tapply(bd$q1,bd$grupo,summary)
tapply(bd$q2,bd$grupo,summary)
tapply(bd$q3,bd$grupo,summary)
tapply(bd$q6,bd$grupo,summary)
...
tapply(bd$dirigia_horas_antes,bd$grupo,summary)
tapply(bd$dirige_horas_atual,bd$grupo,summary)
plota_boxplot(col_box)
```
...
```{r}
tapply(bd$dirigia_dias_antes,bd$grupo,summary)
tapply(bd$dirige_dias_atual,bd$grupo,summary)
```
...
``` {r}
tapply(bd$meem,bd$grupo,summary)
```
